...
const Members = () => {
  ...
  const handleApprove = async (id, status) => {
    setIsLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/members/status-update/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        const updatedResponse = await fetch("http://localhost:5000/api/members/display");
  ...
  const handleExcelUpload = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("http://localhost:5000/api/export/upload-excel", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownload = async (type) => {
    ...
    if (type === "excel") {
      window.open("http://localhost:5000/api/export/download/excel");
    } else if (type === "pdf") {
      window.open("http://localhost:5000/api/export/download/pdf");
    }
  };
