...
function AdSlotBookingSystem() {
  ...
  const fetchSlots = async () => {
    const res = await fetch("http://localhost:5000/api/advertisement/available");
  ...
  const addToDatabase = async (side, transactionId) => {
    const res = await fetch("http://localhost:5000/api/advertisement/book", {
      method: "POST",
  ...
  const handleSubmit = async (e, side) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`http://localhost:5000/api/advertisement/add-advertise-image/${side}`, {
        method: 'PUT',
        body: formData
      });
      const data = await response.json();
